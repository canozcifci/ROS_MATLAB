...
odom_pos_x = [];     
odom_pos_y = [];      
while rate.TotalElapsedTime < 5            % Get laser scan data          
...
targetDir = (r_max-r_min).*rand();          % Call VFH object to computer steering direction          
steerDir = vfh(ranges, angles, targetDir);          
ob_dist = [ob_dist, min(ranges) ];          % Calculate velocities          
